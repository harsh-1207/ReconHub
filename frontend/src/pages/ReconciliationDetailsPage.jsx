...
export default function ReconciliationDetailsPage() {
  ...
  if (!reconciliation)
    return <Alert type="error">{error || "Reconciliation not found."}</Alert>;
    ...
    <>
      ...
      <Card>
        <div className="document-pair">
          <div>
            <span>Actual</span>
            <strong>{reconciliation.actual_document_name}</strong>
          </div>
          <div className="pair-arrow">→</div>
          <div>
            <span>Filed</span>
            <strong>{reconciliation.filed_document_name}</strong>
          </div>
        </div>
      ...
      <Card
        title={`Exceptions (${reconciliation.exceptions?.length || 0})`}
        subtitle="Only mismatched fields and missing/extra line items appear here."
      >
        <ExceptionsTable rows={reconciliation.exceptions} />
      </Card>
      <Card
        title={`All comparisons (${reconciliation.results?.length || 0})`}
        subtitle="Deterministic comparison results stored for this reconciliation."
      >
