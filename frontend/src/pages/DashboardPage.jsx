...
export default function DashboardPage() {
  ...
  const metrics = useMemo(
    () => ({
      matched: reconciliations.filter((item) => item.status === "MATCHED")
        .length,
      exceptions: reconciliations.filter(
        (item) => item.status === "EXCEPTIONS_FOUND",
      ).length,
      averageMatch: reconciliations.length
        ? reconciliations.reduce(
            (sum, item) => sum + Number(item.match_percentage || 0),
            0,
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Link className="button button-primary" to="/reconcile">
            New reconciliation
          </Link>
        }
      />
      ...
      <div className="dashboard-grid">
        <Card
          ...
          subtitle="Latest comparison runs"
          actions={
        ...
        <Card title="System snapshot">
          <div className="snapshot-list">
            <div>
              ...
              <strong>{metrics.averageMatch.toFixed(2)}%</strong>
            </div>
            <div>
              <span>Supported formats</span>
              <strong>CSV · XLS · XLSX · JSON</strong>
            </div>
            <div>
