...
export default function ReconciliationForm({ documents, onRun, busy }) {
  ...
  const actualDocument = useMemo(
    () => documents.find((item) => String(item.id) === String(actualId)),
    [documents, actualId],
  );
  const filedDocument = useMemo(
    () => documents.find((item) => String(item.id) === String(filedId)),
    [documents, filedId],
  );
    ...
    <form className="reconciliation-form" onSubmit={submit}>
      <div className="compare-grid">
        <label className="field">
          ...
          {actualDocument && (
            <small>
              {String(actualDocument.file_type).toUpperCase()} document
            </small>
          )}
        </label>
        ...
        <label className="field">
          ...
          <select
            value={filedId}
            onChange={(event) => setFiledId(event.target.value)}
          >
          ...
          {filedDocument && (
            <small>
              {String(filedDocument.file_type).toUpperCase()} document
            </small>
          )}
        </label>
      </div>
      {actualId && filedId && actualId === filedId && (
        <p className="field-error">Select two different documents.</p>
      )}
      <div className="form-footer">
        <p>
          The engine compares canonical header fields and order-independent line
          items.
        </p>
        <button
