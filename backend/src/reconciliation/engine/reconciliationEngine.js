...
const headerFields = ['invoiceNumber', 'vendor', 'date', 'currency', 'subtotal', 'tax', 'total'];
...
function comparisonResult(category, field, actual, filed, itemDescription = null) {
...
function reconcileDocuments(actual, filed) {
  const results = headerFields.map((field) => comparisonResult('HEADER', field, actual[field], filed[field]));
  ...
  lineMatches.forEach((match, index) => {
    if (match.status === 'MISSING' || match.status === 'EXTRA') {
      ...
      results.push({
        ...
        item_description: actualItem.description || filedItem.description || `Item ${index + 1}`,
    ...
    lineFields.forEach((field) => {
      const result = comparisonResult(
        ...
        match.actual[field],
        match.filed[field],
        match.actual.description || match.filed.description || `Item ${index + 1}`
      );
  ...
  const matchedFields = results.filter((result) => result.comparison_status === 'MATCHED').length;
