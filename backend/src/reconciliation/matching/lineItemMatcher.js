...
function isModified(actual, filed) {
  ...
  return ['quantity', 'unitPrice', 'tax', 'amount'].some((field) => !close(actual[field], filed[field]));
}
...
function chooseStrongCandidate(actual, filedItems, usedFiledIndexes) {
  for (const strategy of strongStrategies) {
    const matches = filedItems
      .map((filed, filedIndex) => ({ filed, filedIndex, score: strategy(actual, filed) }))
      .filter((candidate) => candidate.score > 0 && !usedFiledIndexes.has(candidate.filedIndex));
    if (matches.length) return matches.sort((a, b) => a.filedIndex - b.filedIndex)[0];
  }
  return null;
}

function chooseUniqueDescriptionCandidate(actual, filedItems, usedFiledIndexes) {
  if (!actual.description) return null;
  const matches = filedItems
    .map((filed, filedIndex) => ({ filed, filedIndex }))
    .filter(({ filed, filedIndex }) => !usedFiledIndexes.has(filedIndex)
      && filed.description
...
function matchLineItems(actualItems = [], filedItems = []) {
  ...
  actualItems.forEach((actual, actualIndex) => {
    const candidate = chooseStrongCandidate(actual, filedItems, usedFiledIndexes)
      || chooseUniqueDescriptionCandidate(actual, filedItems, usedFiledIndexes);

    if (!candidate) {
      matches.push({ status: 'MISSING', actual, filed: null, actualIndex, filedIndex: null });
      return;
    }
  ...
  filedItems.forEach((filed, filedIndex) => {
    if (!usedFiledIndexes.has(filedIndex)) {
      matches.push({ status: 'EXTRA', actual: null, filed, actualIndex: null, filedIndex });
    }
  });
