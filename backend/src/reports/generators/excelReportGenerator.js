...
function buffer(report) {
  ...
  appendSheet(workbook, 'Exceptions', report.exceptions.map((item) => ({
  ...
  appendSheet(workbook, 'Comparisons', report.results.map((item) => ({
    Category: item.category,
    Field: item.field_name,
    Item: item.item_description,
    'Item Status': item.item_status,
  ...
  appendSheet(workbook, 'Line Items', report.results
